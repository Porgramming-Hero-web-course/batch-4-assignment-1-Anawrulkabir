// Problem - 2 : Create a TypeScript function removeDuplicates that accepts an array of numbers and returns a new array with duplicates removed. Preserve the original order of elements.
{
  ...
  const removeDuplicates = (array: Array<number>): Array<number> => {
    const arrayWithoutDuplicates: Array<number> = [...new Set(array)]
    return arrayWithoutDuplicates
  }

  //   number 2 method
  const removeDuplicates2 = (array: Array<number>): Array<number> => {
    const arrayWithoutDuplicate: Array<number> = array.filter(
  ...
  const removeDuplicates3 = (array: Array<number>): Array<number> => {
    // steps ==>
    ...
    for (let i = 0; i < len; i++) {
      if (hashArray[i] !== 0) resultArr.push(i)
    }
